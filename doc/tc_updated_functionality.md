...
TC.hpp 提供了完整的按键处理支持，包括特殊按键如方向键、功能键等。在Windows平台上，库使用了 `_getch()` 函数来捕获按键，并对扩展键（如方向键）进行了特殊处理，确保它们能够正确识别。
...
while (!gameOver) {
    if (_kbhit()) {  // 检查是否有按键按下
        ...
        if (key == 0 || key == 224) {
            key = _getch();
            switch (key) {
                case 72: handleUpKey(); break;    // 上方向键
                case 80: handleDownKey(); break;  // 下方向键
                case 75: handleLeftKey(); break;  // 左方向键
                case 77: handleRightKey(); break; // 右方向键
            }
        } else {
            // 处理普通键
            handleNormalKey(key);
