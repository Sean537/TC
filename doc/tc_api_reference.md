...
| 函数 | 描述 | 参数 | 返回值 |
|------|------|------|--------|
| `terminal::flush()` | 刷新终端输出缓冲区 | 无 | 无 |
| `Printer::flush()` | 刷新打印器的输出缓冲区 | 无 | Printer引用 |

缓冲区控制函数用于确保终端输出的即时性，特别适用于以下场景：
...
// 使用打印器的刷新方法
tc::printer().print("\r加载中...").flush();
```

注意事项：
1. `print` 函数在检测到 `\r` 字符时会自动调用 `flush`
2. `println` 函数会自动刷新输出
3. 在动画效果中，建议每帧后都调用 `flush`
